...
function build_kernel() {
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${bliss_version}")
        if [ -n "${BLISS_MIRROR}" ]; then
            repo_init_args+=("--reference" "${BLISS_MIRROR}")
        fi
        if [ -n "${REPO_VERSION}" ]; then
            repo_init_args+=("--repo-rev" "${REPO_VERSION}")
        fi

        yes | repo init -u https://github.com/BlissRoms-Devices/${target_kernel_manifest}.git ${repo_init_args[@]} || [ $? -eq 141 ]
        if [ $? -ne 0 ]; then
